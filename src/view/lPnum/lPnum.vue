<template>
  <div>
    <el-divider content-position="left">1.在此区域勾选需要分配或修改号码的科室</el-divider>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="备注" prop="lOther" width="120" />
    ...
    <el-divider content-position="left">2.在此区域筛选号码</el-divider>
    <div><phoneNum @selectdata="p_multipleselect"></phoneNum></div>
    ...
    <el-divider content-position="left">3.确认号码并分配</el-divider>
    <el-col :span="11"><el-card class="box-card">
    <template #header>
      <div class="card-header">
        <span>需要号码的科室或人员</span>
      </div>
    </template><!-- 
    <div v-for="o in leaderdata" :key="o" class="text item">{{ o.lNum }}</div> -->
    <el-table :data="multipleSelection" style="width: 100%">
    <el-table-column prop="lNum" label="序号" width="180" />
    <el-table-column prop="lName" label="姓名" width="180" />
    <el-table-column prop="lUnit" label="单位/办公室" />
  </el-table>
  </el-card>
</el-col>

<el-col :span="1">
  <el-button type="primary" @click="distribution()">分配</el-button></el-col>
<el-col :span="12">
  <el-card class="box-card">
    ...
    <el-table :data="p_multiple" style="width: 100%">
    <el-table-column prop="nNumber" label="号码" width="180" />
    <el-table-column prop="uid" label="标识符" width="180" />
    <el-table-column prop="nOther" label="备注" />
  </el-table>
  </el-card>
</el-col>
  </el-row>
...
import phoneNum from "../phoneNum/phoneNum.vue";
...
//===========子组件传值部分============
const p_multiple = ref([])

const p_multipleselect = (value)=>{
  p_multiple.value = value; 
}

//============分配号码=============
const distribution = async ()=>{
  if(multipleSelection.value.length ==0 || p_multiple.value.length ==0|| multipleSelection.value.length != p_multiple.value.length){ //判断是否有选中人员
    ElMessage({
    message: '请勾选匹配数量的人员和号码',
    type: 'warning',
  })
  }else{
    let count = 0;
    for (let element of multipleSelection.value) {
      console.log(element,p_multiple.value[count].nNumber);
      element.IPhone = p_multiple.value[count].nNumber;
      console.log(element,p_multiple.value[count].nNumber);
      let result;
      result =  await updateLPhone(element);
      count++;
